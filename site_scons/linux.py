...
def CompileFramework(env, objs, headers, libs, framework_structure):
  app_env = env.Clone()
  libs = ['opengl32','glu32','dinput','dxguid','winmm','freetype235','jpeg6b', 'fmodex']
  app_env.Append(LIBS = libs, LIBPATH = ['Glop/Linux/lib'])
  ...
  #app_env.Append(LINKFLAGS = ['-static'])
...
def Application(env, target, source, resources = [], frameworks = []):
  ...
  l = ['GL', 'GLU', 'freetype', 'jpeg', 'fmodex']
  app_env.Append(LIBS = ["GlopDbg"] + l)
  app_env.Append(LIBPATH = [os.path.dirname(Glop.get_abspath())])
  app_env.Append(LIBPATH = ["../Glop/Linux/lib"])
  app_env.Append(LIBPATH = ["/usr/lib"])
